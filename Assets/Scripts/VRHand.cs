...
//A enum created to mark the handedness.
...
public class VRHand : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        //Deal with this triggerEnter only when the othe object has VRInteractable.
        var vrHandInterButton = other.gameObject.GetComponent<VRHandInteractable>();
        ...
        {   
            ...
            if (currentInteractableButton == null)
            {
                ...
                //call the OnHandOver method on the current interactable as the hand is touching the object now.
            ...
            {
                //Call the OnHandOut method on the previous interactable as the hand has left the previous object.
                currentInteractableButton.OnHandOut();
                //Update the current interactable reference.
                currentInteractableButton = vrHandInterButton;
                //Call the OnHandOver method on the newly touched interactable as the hand is touching the new object now.
                currentInteractableButton.OnHandOver();
            }
    ...
    {
        //Deal with this triggerExit only when the othe object has VRInteractable.
        var vrHandInterButton = other.gameObject.GetComponent<VRHandInteractable>();
        //If the current vr interactable is not empty
        if (vrHandInterButton != null)
        {
            //This is for the task of next week.
            //When the hand leaves the interactable's collider, a "pressed" action is triggered.
            //vrHandInterButton.OnPressed();

            //Call the OnHandOut method on the current interactable is the hand is leaving the other object.
            vrHandInterButton.OnHandOut();
            //Clears the current interactable as the hand is not touching anything now.
